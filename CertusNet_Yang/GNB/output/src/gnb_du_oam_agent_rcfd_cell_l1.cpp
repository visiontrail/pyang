...
oam_agent_rcfd_cell_l1::oam_agent_rcfd_cell_l1(XCONFD_YANGTREE_T* yt)
{
    ...
    xconfd_get(ssb_pwr_, uint32, "ssb-pwr", yt);
    xconfd_get(ssb_prb_offset_, uint8, "ssb-prb-offset", yt);
    xconfd_get(ca_lvl_, uint8, "ca-lvl", yt);
    xconfd_get(ul_carrier_k0_, enum, "ul-carrier-k0", yt);
    xconfd_get(dl_carrier_k0_, enum, "dl-carrier-k0", yt);
...
void oam_agent_rcfd_cell_l1::read_grp_mib(XCONFD_YANGTREE_T* yt, Mib& mib)
{
    ...
    xconfd_get(mib.cell_barred, enum, "cell-barred", yt);
    xconfd_get(mib.intra_freq_resel, enum, "intra-freq-resel", yt);
...
void oam_agent_rcfd_cell_l1::read_grp_prach(XCONFD_YANGTREE_T* yt, Prach& prach)
{
    ...
    xconfd_get_optional_enum(prach.restricted_set, RestrictedSetCfgE, "restricted-set", yt);
